...
public class PPAutoDynamic extends CommandBase {
  ...
  public void initialize() {
    ...
    if((preplacedList.size()+1)-pinList.size() >= 0){
      ...
      Node startingPoint = new Node(PoseEstimator.getCurrentPose().getX(),FieldConstants.fieldWidth-PoseEstimator.getCurrentPose().getY(),PoseEstimator.getCurrentPose().getRotation());
      ...
      for (int i = 0; i < (preplacedListSize); i++) {
        ...
        if(i%2 != 1){
          ...
          BestPath = FindTrajectory.getPathTrajectoryAuto(Drivetrain, PoseEstimator, constraints,obstacles,AStarMap,startingPoint,preplacedList.get(0).getNode());
          
          for (int j = 0; j < preplacedList.size(); j++) {
            
            currentPath = FindTrajectory.getPathTrajectoryAuto(Drivetrain, PoseEstimator, constraints,obstacles,AStarMap,startingPoint,preplacedList.get(j).getNode());
